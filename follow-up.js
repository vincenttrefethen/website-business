...
function buildFollowUpUrl(phone, lead) {
  ...
  return {
    smsUrl: `sms:+${phone}?body=${encodeURIComponent(text)}`,  // primary — Phone Link
...
function buildFollowUpHtml(entries, generatedAt) {
  ...
  const rows = entries.map((e, i) => `
    <div class="card" id="card-${i}">
      ...
      <a class="btn-wa" href="${e.smsUrl||e.waUrl}" target="_blank" rel="noopener" style="background:#2563eb"
         onclick="markSent(${i})">
  ...
  const allUrls = JSON.stringify(entries.map(e => e.smsUrl || e.waUrl));
...
async function followUp() {
  ...
  due.forEach(lead => {
    ...
    const { smsUrl, waUrl, version: fuVer } = buildFollowUpUrl(phone, lead);
    lead.message_version = fuVer;
    entries.push({ ...lead, smsUrl, waUrl });
